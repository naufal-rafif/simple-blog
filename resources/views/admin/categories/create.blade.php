...
<div class="container mx-2">
    <div class="block md:flex justify-between items-center">
        <div class="mb-3">
            <h2 class="font-semibold text-2xl">Create Category</h2>
        </div>
    </div>
</div>

<div class="card mb-8">
    <form action="{{ route('categories.store') }}" method="POST" autocomplete="off" id="formUpload">
        @csrf
        <div class="flex flex-wrap">
            <div class="basis-full md:basis-1/2">
                <div class="px-5 mb-5">
                    <label for="" class="mb-2">Name</label>
                    <input name="name" id="" class="text-input px-2 mt-1 w-full" placeholder="Add name" value="{{old('name')}}" required/>
                    @if($errors->has('name'))
